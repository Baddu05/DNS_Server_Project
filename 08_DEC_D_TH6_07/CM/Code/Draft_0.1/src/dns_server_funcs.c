...
int Search_Record(char *filename,char *buf,unsigned short type,int *count)
{
		...
		switch(qinfo->qtype)
		{
			...
			case T_PTR:/* case for DNS Inverse query saved in records file as PTR records */
				...
				if(0 == strncmp((char *)host,file_res.rdata,strlen((char *)host)))
				{
					...
					/* ssign the records fields to resource record structure fields to send as a response to client */
...
void assign_record(struct RES_RECORD *rec, char *request,char *response,struct RES_RECORD *file_res)
{
...
/*void DomainToIP(char *domain)
{
	...
	for(i = 3;i >= 0; i--)
	{
		strncat(domain,token[i],strlen(token[i])+1);
		strcat(domain,".");
	}
...
int Authenticate(int sd,struct sockaddr_storage *clientaddr)
{
...
int update_res_record(char *filename,char *buffer,unsigned char update_option)
{
