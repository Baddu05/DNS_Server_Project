...
void Update(
		...
		struct sockaddr_in *nameserver, 	//socket structure type
		int update_option,			//update option for addition of records
		char *domain_name,			//host name 
		char *ip				//ip address
	)
...
int create_update_message(
				char *buffer,			//to contain packet
				int update_option,		//option for addition or deletion
				char *domain_name,		// host name
				char *ip			//ip address
			)
...
void update_local_cache(
			char *filename,		//file_name of local cache
			int update_option,	//option for addition or deletion
			char *domain_name,	// host
			char *ip		// ip address
			)
...
char *remove_prefix(
		    char *input	// domain name as input from command line
		   )
...
void Dns_Query(int sd,				/*socket descriptor*/
	       struct sockaddr_in *nameserver,	//socket  structure type
	       char *query,			// ip or domain name
	       int option			// 1 for ip or 2 for domain 
	      )
...
char *search_local_cache(
			char *file,	// local cache file name
			char *query,	// ip or domain name
			int option	// 1 for ip or 2 for domain name
			)
